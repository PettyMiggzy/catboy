...
const metaCache = {};
async function meta(pool) { if (metaCache[pool] !== undefined) return metaCache[pool]; try { const t0 = (await pub.readContract({ address: pool, abi: POOL_ABI, functionName: "token0" })).toLowerCase(); const t1 = (await pub.readContract({ address: pool, abi: POOL_ABI, functionName: "token1" })).toLowerCase(); const fee = await pub.readContract({ address: pool, abi: POOL_ABI, functionName: "fee" }); const wethIsT0 = t0 === WETH; const other = wethIsT0 ? t1 : (t1 === WETH ? t0 : null); return metaCache[pool] = other ? { token: other, wethIsT0, fee } : null; } catch { return metaCache[pool] = null; } } // was: dropped every pool where WETH sorts as token0 (~half of all pools)
