...
(function () {
  const CFG = {
    // fill these at launch (from the candy-machine deploy); when set, ownership
    // is verified on-chain automatically.
  ...
  const LS_PREVIEW = "catboy_preview_holder";   // 'genesis' | 'nine' | ''
  const SS_STATE   = "catboy_holder_state";

  const empty = { connected: false, address: "", genesis: 0, nine: 0, total: 0,
                  tier: "none", isHolder: false, isGenesis: false, source: "none" };
  ...
  function tierOf(genesis, nine) {
    if (genesis > 0) return "genesis";
    if (nine > 0) return "nine";
    return "none";
  }
  function apply(genesis, nine, address, source) {
    state.genesis = genesis; state.nine = nine; state.total = genesis + nine;
    state.address = address || state.address; state.source = source;
    state.tier = tierOf(genesis, nine);
  ...
  async function countHoldings(owner) {
    ...
    if (!CFG.genesisMint && !CFG.nineMint) return { genesis: 0, nine: 0, verifiable: false };
  ...
  window.CATBOY_PERKS = {
    // coin-flip win chance (base 0.48 = 2% house edge)
    flipChance(h) { h = h || get(); return h.isGenesis ? 0.53 : h.isHolder ? 0.50 : 0.48; },
    // daily-bonus multiplier
    dailyMult(h)  { h = h || get(); return h.isGenesis ? 2.0 : h.isHolder ? 1.5 : 1.0; },
    // fighter wager payout bonus added to the base multiplier
    wagerBonus(h) { h = h || get(); return h.isGenesis ? 0.5 : h.isHolder ? 0.25 : 0; },
    // max wager cap (credits) — holders unlock BIGGER wagers (base stays 100)
    wagerCap(h)   { h = h || get(); return h.isGenesis ? 500 : h.isHolder ? 250 : 100; },
    // fighter skin id unlocked
    skin(h)       { h = h || get(); return h.isGenesis ? "genesis" : h.isHolder ? "nine" : "base"; },
    label(h) {
      h = h || get();
